...
import geva.Operator.FitnessEvaluator;
 ...
 * In Tutorial2 we had a look at a standard GA algorithm 
 * whitout any fitness evaluation. In this tutorial we will show how to
 * add a simple string matching fitness evaluation to the algorithm
 *
...
public class Tutorial7 extends AbstractRun {
	 ...
	 * A fitness function impelemts the interface FitnessFunction, this interface requires it to implement getFitness(Individual i). 
	 ...
	 * valid mappings will be sent to the fitness function. (In GE the individual can be invalid if the there are unfinished non-terminals. 
	 * this can occur when the wrapping of the genotype is to low. Implying that either the genotype is to short or the wrapping number too low.)
	 * 
	...
	pipelineLoop.addModule(fitnessEvaluator); //Evaluate the setPopulation. Here the population will be assigned new fitness
